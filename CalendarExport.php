...
class CalendarExport extends AbstractExternalModule {
    ...
    static function getEvents($month, $year, $pid) {
        return db_query('SELECT * FROM redcap_events_calendar WHERE MONTH(event_date) = ? AND YEAR(event_date) = ? AND project_id = ?',
            [$month, $year, $pid]);
    }

    static function getEvent($cal_id) {
        return db_fetch_assoc(db_query('SELECT * FROM redcap_events_calendar WHERE cal_id = ?', [$cal_id]));
    }
    ...
    static function decodeEvent($eventResult) {
        ...
        $returnResults['project_name'] = db_fetch_assoc(db_query('SELECT app_title FROM  redcap_projects WHERE project_id = ?',
            [$eventResult['project_id']]))['app_title'];

        // get event information
        $returnResults['event_description'] = db_fetch_assoc(db_query('SELECT descrip FROM  redcap_events_metadata WHERE event_id = ?',
            [$eventResult['event_id']]))['descrip'];

        // get group information
        $returnResults['group_name'] = db_fetch_assoc(db_query('SELECT group_name FROM  redcap_data_access_groups WHERE group_id = ?',
    ...
    static function hasAccessToDAG($user, $project, $dag) {
        if (self::$userDAGs === NULL) {
            $dbResult = db_query('SELECT group_id FROM redcap_user_rights WHERE project_id = %d AND username = "%s"', [$project, $user]);
